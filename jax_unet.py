...
def load_doubleconv_weights(jax_doubleconv, torch_state_dict, torch_prefix=""):
    ...
    for pattern in patterns:
        ...
        if matches > 0:
            keys_found.update({k: v for k, v in pattern.items() if v in torch_state_dict})
    ...
    if "conv1_weight" in keys_found:
        ...
        jax_doubleconv.conv1.kernel = jnp.array(weight.transpose(2, 3, 1, 0))  # OIHW -> HWIO
    ...
    if "conv2_weight" in keys_found:
        ...
        jax_doubleconv.conv2.kernel = jnp.array(weight.transpose(2, 3, 1, 0))  # OIHW -> HWIO
...
class unet(nnx.Module):
    def __init__(self, num_layers, in_channels, out_channels, features, kernel_size, stride, padding):
        ...
        for i in range(num_layers-1, 0, -1):
            in_ch = features[i]
            out_ch = features[i-1]
            self.up.append(nnx.ConvTranspose(in_features=in_ch, out_features=out_ch, kernel_size=(2, 2), strides=2, rngs=rngs, transpose_kernel=True))
        ...
        self.output = nnx.Conv(in_features=features[0], out_features=out_channels, kernel_size=kernel_size, strides=stride, padding=padding, rngs=rngs)
    ...
    def load_torch_weights(self):
        ...
        for i in range(self.num_layers):
            ...
            self.encoder[i] = load_doubleconv_weights(self.encoder[i], model_state_dict, torch_prefix)
            
        # Load decoder weights
        for i in range(self.num_layers-1):
            # In PyTorch, decoder layers are typically named in reverse order
            torch_prefix = f"decoder.{i}."
            self.decoder[i] = load_doubleconv_weights(self.decoder[i], model_state_dict, torch_prefix)
        ...
        if "dynamics.weight" in model_state_dict:
            ...
            self.dynamics.kernel = jnp.array(weight.transpose(2, 3, 1, 0))  # OIHW -> HWIO
            if "dynamics.bias" in model_state_dict:
                self.dynamics.bias = jnp.array(model_state_dict["dynamics.bias"].detach().cpu().numpy())
            
        if "statics.weight" in model_state_dict:
            weight = model_state_dict["statics.weight"].detach().cpu().numpy()
            self.statics.kernel = jnp.array(weight.transpose(2, 3, 1, 0))  # OIHW -> HWIO
            if "statics.bias" in model_state_dict:
                self.statics.bias = jnp.array(model_state_dict["statics.bias"].detach().cpu().numpy())
        ...
        if key_prefix + "bias" in model_state_dict:
            self.output.bias = jnp.array(model_state_dict[key_prefix + "bias"].detach().cpu().numpy())
        ...
        for i in range(self.num_layers-1):
            ...
            if key_prefix + "bias" in model_state_dict:
                self.up[i].bias = jnp.array(model_state_dict[key_prefix + "bias"].detach().cpu().numpy())
