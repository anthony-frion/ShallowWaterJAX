...
class UNET2D(torch.nn.Module):
    def __init__(self, 
        ...
        self.dynamics = nn.Conv2d(in_channels[0], int(features[0]/2), kernel_size, stride, padding)
        self.statics = nn.Conv2d(in_channels[1], int(features[0]/2), kernel_size, stride, padding)
        ...
        for i in range(num_layers):
            ...
            out_ch = features[i]
            self.encoder.append(DoubleConv2D(in_channels=in_ch, out_channels=out_ch, kernel_size=kernel_size, stride=stride, padding=padding, num_groups=out_ch//4, norm_type=self.norm_type, activation=self.activation))
        ...
        for i in range(num_layers-1, 0, -1):
            self.up.append(nn.ConvTranspose2d(features[i], features[i-1], kernel_size=2, stride=2))
        
        # Decoder (upsampling) blocks
        self.decoder = nn.ModuleList()
        for i in range(num_layers-1, 0, -1):
            self.decoder.append(DoubleConv2D(in_channels=features[i], out_channels=features[i-1], kernel_size=kernel_size, stride=stride, padding=padding, num_groups=features[i-1]//4, norm_type=self.norm_type, activation=self.activation))
        
        # Output layer
        if len(out_channels) == 1:
            self.output = nn.Conv2d(features[0], out_channels[0], kernel_size, stride, padding)
        else:
            print("Multiple output channels")
            self.output = nn.ModuleList()
            for i in range(len(out_channels)):
                self.output.append(nn.Conv2d(features[0], out_channels[i], kernel_size, stride, padding))
    ...
    def forward(self, x, parameters=None, **kwargs):
        ...
        for i in range(self.num_layers - 1):
            ...
            if x.size(2) != skip_connections[i].size(2) or x.size(3) != skip_connections[i].size(3):
